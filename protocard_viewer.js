...
app.get('/printCards', async function(req,res){
    var name = 'SELECT name FROM images';
    var img = 'SELECT url from images';

    const dbname =  await dbConfig.query('SELECT name FROM images')
    console.log(dbname)
    res.render('printCards');
    res.json({title: "name", name: dbname.rows});
});
